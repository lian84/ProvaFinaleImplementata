...
import com.example.Repository.MovimentiRepository;
...
public class ArticoliController {
	...
	public String saveArtCOntroller(@ModelAttribute ("articolo")Articoli articoli) {
	 ...
	 @GetMapping("/articoli/{id}/movimenti")
	    public String getMovimentiArticolo(@PathVariable("id") String id, Model model) {
	        Articoli articolo = articoliRepository.findById(id).orElse(null);
	        if (articolo != null) {
	            List<Movimenti> movimenti = movimentiRepository.findByArticolo(articolo);
	            model.addAttribute("articolo", articolo);
	            model.addAttribute("movimenti", movimenti);
	        }
	        return "movimentoArticoli";
	    }
